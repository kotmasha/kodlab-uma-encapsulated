...
def getLog(brain):
    ...
    for snapshot_name, snapshot in brain._snapshots.iteritems():
        ...
        # file.close()
        # file.close()
        file.write(logging_info.get_result())
    file.close()
...
class brain:
    ...
    def add_snapshot(self, name, params, using_agent, using_log):
        discount = params[0]
        #snapshot = UMA_NEW.Agent_Forgetful(discount)
        print_info = ("rt" == self.agent._EXPERIMENT.din(self.agent._MID))
        snapshot = UMA_NEW.Agent_Stationary(discount, print_info)
