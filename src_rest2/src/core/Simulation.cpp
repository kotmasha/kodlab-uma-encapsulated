...
Simulation::Simulation(vector<Sensor*> &sensors, vector<SensorPair*> &sensor_pairs, std::map<string, Sensor*> &sensor_idx, int initial_sensor_size, string log_dir){
	_log_dir = log_dir;
	_log = new logManager(logging::VERBOSE, log_dir, "simulation.txt", typeid(*this).name());
	init_pointers();

	_memory_expansion = .5;
	_log->info() << "Default memory expansion rate is " + to_string(_memory_expansion);

	_initial_sensor_size = initial_sensor_size;
	_log->info() << "Setting the initial sensor size to " + to_string(_initial_sensor_size);

	_sensors = sensors;
	_sensor_pairs = sensor_pairs;
	_sensor_idx = sensor_idx;

	reallocate_memory(_sensors.size());
	create_sensors_to_arrays_index(0, _sensors.size());
	create_sensor_pairs_to_arrays_index(0, _sensors.size());
	copy_sensors_to_arrays(0, _sensors.size());
	copy_sensor_pairs_to_arrays(0, _sensors.size());

	_log->info() << "Simulation created";
}

void Simulation::init_pointers() {
	h_dirs = NULL;
	h_weights = NULL;
	h_thresholds = NULL;
	h_mask_amper = NULL;
...
float Simulation::decide(vector<bool> &signal, double phi, bool active) {//the decide function
	_phi = phi;
	setObserve(signal);
...
void Simulation::set_size(int sensor_size, bool change_max = true) {
...
void Simulation::reallocate_memory(int sensor_size) {
	_log->info() << "Starting reallocating memory";
	free_all_parameters();

	set_size(sensor_size);
...
*/
void Simulation::copy_arrays_to_sensors(int start_idx, int end_idx) {
...
void Simulation::copy_sensors_to_arrays(int start_idx, int end_idx) {
...
void Simulation::create_sensors_to_arrays_index(int start_idx, int end_idx) {
...
*/
void Simulation::copy_sensor_pairs_to_arrays(int start_idx, int end_idx) {
...
void Simulation::copy_arrays_to_sensor_pairs(int start_idx, int end_idx) {
...
void Simulation::create_sensor_pairs_to_arrays_index(int start_idx, int end_idx) {
	for (int i = ind(start_idx, 0); i < ind(end_idx, 0); ++i) {
		try {
...
/*
void Simulation::copy_mask(vector<bool> mask) {
...
*/
void Simulation::pruning(vector<bool> &signal) {
	copy_arrays_to_sensors(0, _sensors.size());
	copy_arrays_to_sensor_pairs(0, _sensors.size());
	...
	create_sensors_to_arrays_index(0, _sensor_size);
	create_sensor_pairs_to_arrays_index(0, _sensor_size);
	copy_sensors_to_arrays(0, _sensor_size);
	copy_sensor_pairs_to_arrays(0, _sensor_size);
...
void Simulation::ampers(vector<vector<bool> > &lists, vector<std::pair<string, string> > &id_pairs) {
	copy_arrays_to_sensors(0, _sensors.size());
	copy_arrays_to_sensor_pairs(0, _sensors.size());
	int success_amper = 0;
	...
	if (_sensors.size() > _sensor_size_max) {
		...
		set_size(_sensors.size(), false);
		//copy just the new added sensors and sensor pairs
		create_sensors_to_arrays_index(_sensors.size() - success_amper, _sensors.size());
		create_sensor_pairs_to_arrays_index(_sensors.size() - success_amper, _sensors.size());
		copy_sensors_to_arrays(_sensors.size() - success_amper, _sensors.size());
		copy_sensor_pairs_to_arrays(_sensors.size() - success_amper, _sensors.size());
	}
}

//the input list size should be larger than 2
void Simulation::amper(vector<int> &list, std::pair<string, string> &uuid) {
	if (list.size() < 2) {
...
void Simulation::delays(vector<vector<bool> > &lists, vector<std::pair<string, string> > &id_pairs) {
	copy_arrays_to_sensors(0, _sensors.size());
	copy_arrays_to_sensor_pairs(0, _sensors.size());
	int success_delay = 0;
	//record how many delay are successful
	...
	if (_sensors.size() > _sensor_size_max) {
		...
		set_size(_sensors.size(), false);
		//copy just the new added sensors and sensor pairs
		create_sensors_to_arrays_index(_sensors.size() - success_delay, _sensors.size());
		create_sensor_pairs_to_arrays_index(_sensors.size() - success_delay, _sensors.size());
		copy_sensors_to_arrays(_sensors.size() - success_delay, _sensors.size());
		copy_sensor_pairs_to_arrays(_sensors.size() - success_delay, _sensors.size());
	}
}
...
*/
void Simulation::gen_thresholds() {
	//malloc the space
...
*/
void Simulation::gen_mask_amper() {
	//malloc the space
...
*/
void Simulation::gen_np_direction() {
	//malloc the space
...
void Simulation::gen_npdir_mask() {
	//malloc the space
...
*/
void Simulation::gen_other_parameters() {
...
void Simulation::setTarget(vector<bool> &signal) {
...
void Simulation::setSignal(vector<bool> &signal) {
	for (int i = 0; i < _measurable_size; ++i) {
		h_signal[i] = signal[i];
	}
...
void Simulation::setSignals(vector<vector<bool> > &signals) {
...
void Simulation::setLoad(vector<bool> &load) {
...
void Simulation::setDists(vector<vector<int> > &dists) {
...
*/
void Simulation::setObserve(vector<bool> &observe) {//this is where data comes in in every frame
...
------------------------------------GET FUNCTION------------------------------------
*/
vector<vector<bool> > Simulation::getSignals(int sig_count) {
...
vector<vector<bool> > Simulation::getLSignals(int sig_count) {
	vector<vector<bool> > results;
...
vector<vector<bool> > Simulation::getNpdirMasks() {
	vector<vector<bool> > results;
...
vector<bool> Simulation::getCurrent() {
	vector<bool> result;
...
vector<bool> Simulation::getPrediction() {
	vector<bool> result;
...
*/
vector<bool> Simulation::getTarget() {
	vector<bool> result;
...
*/
vector<vector<double> > Simulation::getWeight2D() {
...
*/
vector<vector<bool> > Simulation::getDir2D() {
...
*/
vector<vector<bool> > Simulation::getNPDir2D() {
...
*/
vector<vector<double> > Simulation::getThreshold2D() {
...
*/
vector<vector<bool> > Simulation::getMask_amper2D() {
	vector<vector<bool> > result;
...
*/
vector<bool> Simulation::getMask_amper() {
...
*/
vector<double> Simulation::getWeight() {
...
*/
vector<bool> Simulation::getNPDir() {
...
*/
vector<double> Simulation::getThresholds() {
...
*/
vector<double> Simulation::getDiag() {
...
*/
vector<double> Simulation::getDiagOld() {
...
*/
vector<bool> Simulation::getMask() {
	vector<bool> result;
...
*/
vector<bool> Simulation::getObserve() {
	vector<bool> result;
...
*/
vector<bool> Simulation::getObserveOld() {
	vector<bool> result;
...
vector<bool> Simulation::getLoad() {
	vector<bool> result;
...
*/
vector<bool> Simulation::getDown() {
	vector<bool> result;
...
*/
Measurable *Simulation::getMeasurable(int idx) {
	int s_idx = idx / 2;
...
Measurable *Simulation::getMeasurable(string &measurable_id) {
...
SensorPair *Simulation::getSensorPair(Sensor *sensor1, Sensor *sensor2) {
...
*/
MeasurablePair *Simulation::getMeasurablePair(int m_idx1, int m_idx2) {
	int s_idx1 = m_idx1 / 2;
	int s_idx2 = m_idx2 / 2;
...
MeasurablePair *Simulation::getMeasurablePair(string &mid1, string &mid2) {
...
vector<bool> Simulation::getAmperList(string &sensor_id) {
...
vector<string> Simulation::getAmperListID(string &sensor_id) {
	if (_sensor_idx.find(sensor_id) == _sensor_idx.end()) {
		throw CoreException("Cannot find the sensor id " + sensor_id, CoreException::ERROR, status_codes::NotFound);
	}
	Sensor *sensor = _sensor_idx[sensor_id];
