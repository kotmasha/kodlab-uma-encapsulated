...
def start_experiment(stdscr,env_length,burn_in,agent_to_examine,delay_string):
    ...
    #signal is a saltus function
    rescaling=lambda r: 1 if r!=0 else 0
    ...
    #for agent in [RT,LT]:
        #for token in ['plus','minus']:
            ...
            #tmp_sigs=[agent.generate_signal(['x' + str(j)  for j in xrange(ind, X_BOUND)], token) for ind in xrange(X_BOUND)]
            #tmp_sigs = [agent.generate_signal(['x' + str(j) + '*' if j < ind else 'x' + str(j) for j in xrange(X_BOUND)], token) for ind in xrange(X_BOUND)]
            #tmp_sigs = [agent.generate_signal(['x8', 'x9'], token)]
            #delay_sigs.extend(tmp_sigs)
...
def main():
    curses.wrapper(start_experiment,10,2000,'rt','nodelay')
