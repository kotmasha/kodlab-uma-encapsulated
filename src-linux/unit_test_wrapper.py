...
class UMAPyUnitTest:
    ...
    def result_report(self, targets, results, compare_fun = None):
        count = 0
        total = len(targets)
        fail_tests = []
        for i in range(total):
            if compare_fun is None:
                if targets[i] == results[i]:
                    count += 1
                else:
                    fail_res = [i, targets[i], results[i]]
                    fail_tests.append(fail_res)
            elif compare_fun is not None:
                if compare_fun(targets[i], results[i]):
                    count += 1
                else:
                    fail_res = [i, targets[i], results[i]]
                    fail_tests.append(fail_res)
    ...
    def test_wrapper(fun):
        def fun_wrapper(self, test_name, kwargs = None):
            ...
            compare_fun = None
            if 'compare_fun' in kwargs:
                print "Use customized compare function"
                compare_fun = kwargs['compare_fun']
            targets, results = fun(self, test_name, kwargs)
            self.result_report(targets, results, compare_fun)
            print "-----------------" + test_name + "--------------------"
        return fun_wrapper
    ...
    @test_wrapper
    def test_init_mask_amper(self, test_name, test_data = None):
        print "Test is done on device"
        sensor_size = test_data['sensor_size']
        targets = test_data['targets']

        results = [self.CPUTest.TEST_init_mask_amper(sensor_size[i]) for i in range(len(targets))]
        return targets, results

    @test_wrapper
    def test_delay(self, test_name, test_data = None):
        weights = test_data['weights']
        measurable = test_data['measurable']
        measurable_old = test_data['measurable_old']
        last_total = test_data['last_total']
        id = test_data['id']
        targets = test_data['targets']

        results = [self.CPUTest.TEST_delay(weights, measurable, measurable_old, last_total, id[i]) for i in range(len(targets))]
        return targets, results