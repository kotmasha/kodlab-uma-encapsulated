...
def start_experiment(stdscr,burn_in,agent_to_examine,delay_string):
    ...
    MOTION_PARAMS=tuple([
        1.-pow(2,-3),   #memory discounting coefficient
    ...
    def print_state(banner_text,reported_data,id_agent):
        ...
        elapsed_time,agent_reports=reported_data
        ...
        for x,mid in enumerate(agent._SENSORS):
            ...
            WINs.addstr(2,hpos(token,x),namelist[token][x],this_BG)
            for token in ['plus','minus']:
                this_BG=tok_BG[token] if SIG[token].out(x) else REG_BG
                WINs.addstr(vpos[token],hpos(token,x),namelist[token][x],this_BG)
        """
...
curses.wrapper(start_experiment,50,'rt','delay')
