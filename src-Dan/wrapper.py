...
class wrapper:
    def __init__(self,agent,type,threshold,using_worker=False,q=0):
        self.agent=agent
        if type=='EMPIRICAL':
            self.brain=UMA_NEW.Agent_Empirical(threshold,using_worker)
        elif type=='DISTRIBUTED':
            self.brain=UMA_NEW.Agent_Distributed(threshold,using_worker)
        elif type=='DISCOUNTED':
            self.brain=UMA_NEW.Agent_Discounted(threshold,q,using_worker)
