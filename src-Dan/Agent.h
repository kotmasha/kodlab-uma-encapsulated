...
public:
	Agent(int type,double threshold,bool using_worker);
	virtual ~Agent();
	...
	void addSensors(vector<vector<int> > &list);
	void copyNewSensorToDevice(vector<vector<double> > &data);
...
private:
public:
	Agent_Empirical(double threshold,bool using_worker);
...
private:
public:
	Agent_Distributed(double threshold,bool using_worker);
...
private:
	double q;
public:
	Agent_Discounted(double threshold,double q,bool using_worker);
