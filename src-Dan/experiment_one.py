...
def start_experiment(stdscr):
	# world constraints
    X_BOUND=16 #length of the interval environment
    Y_BOUND=16
    RADIUS = 8
    THRESHOLD=1./(4*(RADIUS+1.0)) #learning threshold for ALICE
    ...
    NUMBER_OF_TARGETS = (X_BOUND+1)*(Y_BOUND+1)/((RADIUS**2)/4)
    ...
    for item in templist:
        if sum(x%2 for x in item)<2:
            ALICE._GENERALIZED_ACTIONS.remove(item)

    in_bounds = lambda x: x.real >= 0 and x.real <= X_BOUND and x.imag <= Y_BOUND and x.imag >= 0
   ...
   # state['sig'] as a measurable
    ...
    def print_state(counter,text):
        ...
        for target in TARGET_LIST:
            WIN.addch(Y_BOUND+1-int(np.floor(target.imag)),1+int(np.floor(target.real)),int(84)) # print target (playground)

        WIN.addch(Y_BOUND+1-int(np.floor(EX.state('pos')[0].imag)),1+int(np.floor(EX.state('pos')[0].real)),int(83)) # print ALICE's position
        WIN.addch(Y_BOUND+1-int(np.floor((EX.state('pos')[0]+EX.state('pose')[0]).imag)),1+int(np.floor((EX.state('pos')[0]+EX.state('pose')[0]).real)),int(78)) # print ALICE's "NOSE"
        WIN.overlay(stdscr)
        WIN.noutrefresh()
        curses.doupdate()
    #print ALICE._SIZE
