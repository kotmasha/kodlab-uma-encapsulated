...
def start_experiment(stdscr,agent_to_examine):
    ...
    for ind in xrange(X_BOUND):
        tmp_name='x'+str(ind)
        id_tmp,cid_tmp=EX.register_sensor(tmp_name) #registers the sensor pairs
    ...
    # initialization with log spike:
    #INIT=1.-np.log((1.+dist(START,TARGET))/(X_BOUND+2.))
    
    # initialization with linear peak:
    INIT=1+X_BOUND-dist(START,TARGET)
    EX.construct_measurable(id_distM,distM,[INIT,INIT])

    # progress sensor ("am I better now than in the last cycle?")
    # - $id_navM$ already registered
    def navM(state):
    ...
    #
    ### Initialize agents in the back end
    #
    ...
    # another update cycle to ensure all data structures include the new sensors
