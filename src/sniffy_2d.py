...
def start_experiment(stdscr,agent_to_examine):
    ...
    id_up,id_upc=EX.register_sensor('up',True)
    id_dn,id_dnc=EX.register_sensor('dn',True)
    ...
    def arbiter(state):
        id_list = [id for id in [id_lt, id_rt, id_up, id_dn] if id in state[id_dec][0]]
        if len(id_list) == 0:
            return 0
        else:
            return id_list[rnd(len(id_list))]

    EX.construct_measurable(id_arbiter,arbiter,[id_lt],0)
    ...
    id_toUP,id_toUPc=EX.register_sensor('toU',True)
    def intention_UP(state):
        return id_up in state[id_dec][0]
    EX.construct_sensor(id_toUP,intention_UP)
    
    id_toDN,id_toDNc=EX.register_sensor('toD',True)
    def intention_DN(state):
        return id_dn in state[id_dec][0]
    EX.construct_sensor(id_toDN,intention_DN)

    # failure mode for action $lt^rt$
    id_toF,id_toFc=EX.register_sensor('toF',True)
    def about_to_enter_failure_mode(state):
        return sum([state[id_toLT][0], state[id_toRT][0], state[id_toUP][0], state[id_toDN][0]]) > 1
    ...
    def action_RT(state):
        ...
        if state[id_toF][0]:
            ...
            return state[id_arbiter][0] == id_rt
        else:
            return rt_decided
    ...
    def action_LT(state):
        ...
        if state[id_toF][0]:
            ...
            return state[id_arbiter][0] == id_lt
        else:
            return lt_decided
    LT=EX.construct_agent(id_lt,id_distM,action_LT,MOTION_PARAMS,True)

    def action_UP(state):
        up_decided=(id_up in state[id_dec][0])
        if state[id_toF][0]:
            #return not(rt_decided) if state[id_arbiter][0] else rt_decided
            return state[id_arbiter][0] == id_up
        else:
            return up_decided
    UP=EX.construct_agent(id_up,id_distM,action_UP,MOTION_PARAMS,True)

    def action_DN(state):
        dn_decided=(id_dn in state[id_dec][0])
        if state[id_toF][0]:
            #return lt_decided if state[id_arbiter][0] else not(lt_decided)
            return state[id_arbiter][0] == id_dn
        else:
            return dn_decided
    DN=EX.construct_agent(id_dn,id_distM,action_DN,MOTION_PARAMS,True)
    ...
    START=[rnd(X_BOUND+1), rnd(Y_BOUND)]
    #START=[15,3]
    # effect of motion on position
    id_pos=EX.register('pos')
    def motion(state):
        triggersX={id_rt:1,id_lt:-1}
        triggersY={id_up:1,id_dn:-1}
        diffX=0
        diffY=0
        for t in triggersX:
            diffX+=triggersX[t]*int(state[t][0])
        for t in triggersY:
            diffY+=triggersY[t]*int(state[t][0])
        newposX=state[id_pos][0][0]+diffX
        newposY=state[id_pos][0][1]+diffY
        if not in_Xbounds(newposX):
            newposX=state[id_pos][0][0]
        if not in_Ybounds(newposY):
            newposY=state[id_pos][0][1]
    ...
    def xsensor(m): # along x-axis
        return lambda state: state[id_pos][0][0]<m+1

    def ysensor(m): # along x-axis
        return lambda state: state[id_pos][0][1]<m+1
    ...
    for ind in xrange(X_BOUND):
        ...
        EX.assign_sensor(id_tmp,True,[id_rt,id_lt,id_up,id_dn]) #assigns the sensor to all agents

    for ind in xrange(Y_BOUND - 1):
        tmp_name='y'+str(ind)
        id_tmp,id_tmpc=EX.register_sensor(tmp_name) #registers the sensor pairs
        EX.construct_sensor(id_tmp,ysensor(ind)) #constructs the measurables associated with the sensor
        EX.assign_sensor(id_tmp,True,[id_rt,id_lt,id_up,id_dn]) #assigns the sensor to all agents
    ...
    def distM(state):
        if state[id_pos][0]==TARGET and state[id_pos][1]==TARGET:
            ...
            return 1+X_BOUND+Y_BOUND-dist(state[id_pos][0],TARGET)

    INIT=1+X_BOUND+Y_BOUND-dist(START,TARGET)
    ...
    EX.assign_sensor(id_navM,True,[id_rt,id_lt,id_up,id_dn]) #assigns the sensor to all agents
    ...
    for agent in [RT,LT,UP,DN]:
        delay_sigsX=[agent.generate_signal([EX.nid('x'+str(ind))]) for ind in xrange(X_BOUND)]
        delay_sigsY=[agent.generate_signal([EX.nid('y'+str(ind))]) for ind in xrange(Y_BOUND - 1)]
        agent.delay(delay_sigsX + delay_sigsY)
    ...
    WIN=curses.newwin(7+Y_BOUND,2*X_BOUND+3,5,7)
    ...
    def print_state(text,id_agent):
        ...
        for ind,lookat in enumerate([curr,pred,targ]): 
            ...
            tok_line={'plus':3-ind,'minus':4+Y_BOUND+ind}
            for token in ['plus','minus']:
        ...
        WIN.addch(4+TARGET[1],1+2*TARGET[0],ord('T'),FG)
        # display agent's position with FG attributes
        WIN.addch(4+EX.this_state(id_pos,1)[1],1+2*EX.this_state(id_pos,1)[0],ord('S'),FG)
